...
def libro(query):
    ...
    qa_dict = {
        "Who are you?": "I am LibroBot.",
        "How are you?": "I'm just a program, so I don't have feelings, but thanks for asking!",
        "is this book available":"book_availability",
        "when should i return my book":"book_return",
        "how much limit am having yet what is my limit":"my_limit",
        "how to issue books":"You just have to scan the book,then enter your pin ",
        # Add more questions and answers as needed
    }

    # Split the query into words and find the answer with the most matching words
    query_words = query.lower().split()
    max_matching_words = 0
    best_answer = "I'm sorry, I don't understand that question."

    for question, answer in qa_dict.items():
        question_words = question.lower().split()
        matching_words = sum(word in query_words for word in question_words)
...
def chat():
    ...
    res=libro(requested_query)
    if(res=='book_availability'):
        conn = mysql.connector.connect(**config)
        cursor = conn.cursor()
        query="select title from books"
        cursor.execute(query)
        books=cursor.fetchall()
        books=[book[0].lower() for book in books]
        print(books)
        book_name=''

        for word in requested_query.split(' '):
            print(word)
            if word in books:
                book_name=word
                print('found book name,it is ',book_name)
                break
        if(book_name):
            query=f"select isissued from books where title='{book_name}'"
            cursor.execute(query)
            rows=cursor.fetchall()
            ...
            for i in rows:
                if i[0]==1:
                    ...
                    isissued=0
            if(isissued==0):
                res=book_name,'is available in our library'
            else:
                res=book_name,'is issued by some one'
        if(book_name==''):
            res='your requested book is not available in our library'
...
def prebook():
    ...
    try:
        query=f"select title from books where title='{bname}'"
        cursor.execute(query)
        rows=cursor.fetchall()
        if rows:
            query=f"insert into bookings values('{roll}','{bname}')"
            cursor.execute(query)
            conn.commit()
            res=f"{bname} is prebooked for you"
        else:
            res=f"{bname} is not available"
    except Exception as e:
        res="error"
    return jsonify({'prebook': res})
